...
v1 generates risk with a formula. v2 **learns to predict it** from signals.

The LSTM (Long Short-Term Memory) takes a 10-step window of four signals per edge and predicts next-step risk across all 24 edges simultaneously:
...
The tabular Q-table has a scaling problem. With 24 edges and risk at 5 discretisation levels, the state space is 19 × 5²⁴ ≈ **60 trillion states**. The agent visits 0.00003% of them during training. At inference time it hits unseen states with Q=0 everywhere and produces meaningless paths.

DQN replaces the table with a neural network:

```
Input (43 dims):    one-hot(node, 19) + risk_vector(24)
                              ↓
           Linear(43→256) + LayerNorm + ReLU
                              ↓
                   Linear(256→128) + ReLU
                              ↓
                      Linear(128→19)
                              ↓
              Q-value for each of 19 possible next nodes
```

Two stabilisation tricks make this trainable:

**Experience replay** — store 10,000 past transitions, sample 64 randomly per update. Breaks temporal correlation between consecutive training samples.

**Target network** — a frozen copy of the policy network, synced every 100 gradient steps. Prevents Q-targets from chasing a moving bullseye.
